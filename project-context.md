...
server/
  ...
  js/
    ...
    shared/
      ...
      task-scheduler.mjs
  scripts/
    build-media-index.mjs
    watch-media-index.mjs
  config.json
  out.mp4
  package.json
  project-context.md
...
## File: client/js/app/core/date-utils.js
````javascript
...
## File: client/js/app/infrastructure/favorites.repository.js
````javascript
...
## File: client/js/app/state/store.js
````javascript
...
## File: client/js/media-loader.js
````javascript
function delay(ms)
⋮----
...
````json
{
  "remoteUrl": "http://192.168.1.125:3000"
}
````
...
````json
{
  "name": "media3xv-client",
  "private": true,
  "type": "module",
  "main": "electron/main.mjs",
  "scripts": {
    "start": "electron electron/main.mjs",
    "dev": "electron electron/main.mjs"
  },
  "devDependencies": {
    "electron": "^33.3.1"
  }
...
## File: client/project-context.md
````markdown
This file is a merged representation of the entire codebase, combined into a single document by Repomix.
The content has been processed where comments have been removed, empty lines have been removed, content has been compressed (code blocks are separated by ⋮---- delimiter).
...
<div id="viewer">
  ...
  <div id="viewerStage">
    ...
    <div id="viewerLivePhotoPlayer" style="display:none;width:92vw;max-width:92vw;height:82vh;max-height:82vh;"></div>
...
<div id="cfgModal" style="display:none;position:fixed;inset:0;z-index:85;background:rgba(0,0,0,.65);backdrop-filter:blur(2px);">
  <div style="max-width:680px;max-height:84vh;margin:8vh auto;background:var(--panel);border:1px solid var(--border);border-radius:12px;overflow:hidden;display:flex;flex-direction:column;">
    ...
    <div style="padding:14px;display:flex;flex-direction:column;gap:10px;overflow-y:auto;flex:1;min-height:0;">
      ...
      <label style="font-size:12px;color:var(--muted-l);">remoteUrl Server</label>
      <input id="cfgRemoteUrl" type="text" placeholder="http://192.168.1.125:3000" style="width:100%;background:var(--card);border:1px solid var(--border);color:var(--text);border-radius:8px;padding:8px;font-family:var(--mono);font-size:12px;">
      ...
      <div style="display:flex;align-items:center;gap:8px;justify-content:flex-start;flex-wrap:wrap;padding-top:8px;border-top:1px solid var(--border);">
        <button class="tbtn" onclick="loadPartitionsList()">Cargar particiones</button>
      </div>
      <div id="partitionsBox" style="max-height:220px;overflow:auto;border:1px solid var(--border);border-radius:8px;background:var(--card);display:flex;flex-direction:column;gap:4px;padding:6px;flex-shrink:0;">
        <div style="font-size:11px;color:var(--muted);">Sin datos. Presiona "Cargar particiones".</div>
      </div>
...
⋮----
async function reindexPartitionByKey(partitionKey)
⋮----
async function loadPartitionsList()
⋮----
btn.onclick = ()
⋮----
...
⋮----
function clearLivePrepareRetryTimer()
⋮----
function destroyLivePhotoKitPlayer()
⋮----
...
⋮----
async function clearThumbDb()
⋮----
req.onblocked = ()
⋮----
async function ensureThumbCacheForCurrentBase()
⋮----
...
⋮----
function forceGridReloadBeforeRender()
⋮----
...
## File: server/client-http/js/app/core/date-utils.js
````javascript
export function dateRank(dateStr)
⋮----
export function monthGroupKey(dateStr)
...
## File: server/client-http/js/app/infrastructure/favorites.repository.js
````javascript
export function createFavoritesRepository(storageKey)
⋮----
const safeParse = (value) =>
...
## File: server/client-http/js/app/state/store.js
````javascript
export function createStore(initial =
⋮----
getState()
...
## File: server/client-http/js/media-loader.js
````javascript
function delay(ms)
⋮----
function bustCache(url)
...
## File: server/client-http/index.mjs
````javascript
...
## File: server/js/features/cache/service.mjs
````javascript
...
## File: server/js/shared/task-scheduler.mjs
````javascript
...
## File: server/scripts/build-media-index.mjs
````javascript
...
## File: server/scripts/watch-media-index.mjs
````javascript
function runBuild()
⋮----
...
## File: server/project-context.md
````markdown
This file is a merged representation of the entire codebase, combined into a single document by Repomix.
The content has been processed where comments have been removed, empty lines have been removed, content has been compressed (code blocks are separated by ⋮---- delimiter).
...
# Directory Structure
```
client-http/index.html
client-http/index.mjs
client-http/js/app/application/apply-filters.usecase.js
client-http/js/app/config.js
client-http/js/app/core/date-utils.js
client-http/js/app/infrastructure/favorites.repository.js
client-http/js/app/main.js
client-http/js/app/state/store.js
client-http/js/media-loader.js
config.json
js/features/cache/service.mjs
js/features/config/repository.mjs
js/features/indexing/builders.mjs
js/features/live/service.mjs
js/features/thumb/handler.mjs
js/shared/media-utils.mjs
js/shared/task-scheduler.mjs
package.json
...
## File: client-http/js/app/application/apply-filters.usecase.js
```javascript
export function applyFiltersUseCase(
⋮----
const isLivePhoto = f
...
## File: client-http/js/app/config.js
```javascript
const withBase = (p) => `$
```

## File: client-http/js/app/core/date-utils.js
```javascript
export function dateRank(dateStr)
⋮----
export function monthGroupKey(dateStr)
...
## File: client-http/js/app/infrastructure/favorites.repository.js
```javascript
export function createFavoritesRepository(storageKey)
⋮----
const safeParse = (value) =>
...
## File: client-http/js/app/main.js
```javascript
...
## File: client-http/js/app/state/store.js
```javascript
export function createStore(initial =
⋮----
getState()
...
## File: client-http/js/media-loader.js
```javascript
function delay(ms)
⋮----
function bustCache(url)
...
## File: js/features/cache/service.mjs
```javascript
function createCacheService(
⋮----
async function safeClearDir(targetDir)
...
## File: js/features/config/repository.mjs
```javascript
...
## File: js/features/indexing/builders.mjs
```javascript
...
## File: js/features/live/service.mjs
```javascript
...
## File: js/features/thumb/handler.mjs
```javascript
async function runCommand(command, args)
⋮----
...
## File: js/shared/media-utils.mjs
```javascript
function detectType(name)
⋮----
...
## File: js/shared/task-scheduler.mjs
```javascript
function createTaskScheduler(options =
⋮----
function snapshot()
...
```json
{
  "name": "media3xv-server",
  "private": true,
  "type": "module",
  "scripts": {
    "start": "node server.mjs",
    "start:cluster": "node server.mjs --cluster",
    "dev": "nodemon --watch . --ignore '../.thumb-cache/**' --ignore '../.temp_livephotos/**' --ignore '../.git/**' --ignore '../node_modules/**' server.mjs"
  },
  "devDependencies": {
    "nodemon": "^3.1.10"
  }
...
## File: client/js/app/application/apply-filters.usecase.js
````javascript
export function applyFiltersUseCase(
⋮----
const isLivePhoto = f
...
## File: server/client-http/js/app/application/apply-filters.usecase.js
````javascript
export function applyFiltersUseCase(
⋮----
const isLivePhoto = f
...
## File: server/client-http/js/app/config.js
````javascript
const withBase = (p) => `$
````

## File: server/js/features/config/repository.mjs
````javascript
async function readConfig(fs, configFile)
⋮----
async function writeConfig(fs, configFile, cfg)
...
````json
{
  ...
  "scripts": {
    "start": "node server.mjs",
    "start:cluster": "node server.mjs --cluster",
    "dev": "nodemon --watch . --ignore 'media-index.json' --ignore '../.thumb-cache/**' --ignore '../.temp_livephotos/**' --ignore '../.git/**' --ignore '../node_modules/**' server.mjs"
  },
  "devDependencies": {
    "nodemon": "^3.1.10"
  }
...
@media (max-width: 768px) {
  ...
  #selectModeBtn, #deleteSelectedBtn, #fixSelectedBtn {
    width: 100%;
    justify-content: center;
  }
  #topbar .search-wrap input {
...
@media (max-width: 480px) {
  ...
  #selectModeBtn span#selectModeLbl,
  #deleteSelectedBtn span#deleteSelectedLbl,
  #fixSelectedBtn span#fixSelectedLbl {
    max-width: 84px;
    overflow: hidden;
    text-overflow: ellipsis;
  }
...
@media (max-width: 768px) {
  ...
  #selectModeBtn { grid-area: select; width: 100%; justify-content: center; min-width: 0; }
  #deleteSelectedBtn { grid-area: del; width: 100%; justify-content: center; min-width: 0; }
  #fixSelectedBtn { grid-area: fix; width: 100%; justify-content: center; min-width: 0; }
  .search-wrap { grid-area: search; width: 100%; max-width: 100%; min-width: 0; }
  #topbar > div[style*="flex:1"] { grid-area: spacer; min-width: 0; }
  #topbar .tbtn { min-width: 0; }
  #topbar .tbtn span {
...
<div id="main">
  <header id="topbar">
    ...
    <button class="tbtn" id="fixSelectedBtn" onclick="editSelectedDate()" title="Corregir fecha seleccionada" style="display:none;">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M8 3v4"/><path d="M16 3v4"/><path d="M3 11h18"/><rect x="3" y="5" width="18" height="16" rx="2"/><path d="m9 15 2 2 4-4"/></svg>
      <span id="fixSelectedLbl">Corregir fecha</span>
    </button>
    <button class="tbtn" id="cfgBtn" onclick="openConfigModal()" title="Configuración">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 1 1-4 0v-.09a1.65 1.65 0 0 0-1-1.51 1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 1 1-2.83-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 1 1 0-4h.09a1.65 1.65 0 0 0 1.51-1 1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 1 1 2.83-2.83l.06.06a1.65 1.65 0 0 0 1.82.33h0A1.65 1.65 0 0 0 9.91 3H10a2 2 0 1 1 4 0h.09a1.65 1.65 0 0 0 1 1.51h0a1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 1 1 2.83 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82v0A1.65 1.65 0 0 0 21 9.91H21a2 2 0 1 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg>
    </button>
...
<div id="toast"></div>
<div id="deleteConfirmModal" style="display:none;position:fixed;inset:0;z-index:87;background:rgba(0,0,0,.7);backdrop-filter:blur(2px);">
  <div style="max-width:520px;width:calc(100% - 24px);margin:14vh auto;background:linear-gradient(180deg, rgba(34,18,18,.98), rgba(18,14,14,.98));border:1px solid rgba(220,80,80,.35);border-radius:14px;overflow:hidden;display:flex;flex-direction:column;box-shadow:0 24px 70px rgba(0,0,0,.45);">
    <div style="display:flex;align-items:center;justify-content:space-between;padding:12px 14px;border-bottom:1px solid rgba(220,80,80,.22);">
      <strong style="font-size:13px;color:#ffb7b7;">Eliminar seleccionadas</strong>
      <button class="tbtn" onclick="closeDeleteConfirmModal()">Cerrar</button>
    </div>
    <div style="padding:14px;display:flex;flex-direction:column;gap:12px;">
      <div style="font-size:12px;color:var(--muted-l);line-height:1.45;">
        <div id="deleteConfirmCount" style="font-family:var(--mono);color:var(--text);margin-bottom:6px;">0 archivos</div>
        <div>Esta acción elimina los archivos de la lista actual y no se puede deshacer desde aquí.</div>
      </div>
      <div id="deleteConfirmList" style="max-height:160px;overflow:auto;border:1px solid rgba(220,80,80,.18);background:rgba(0,0,0,.18);border-radius:10px;padding:10px;font-size:12px;color:var(--muted-l);display:flex;flex-direction:column;gap:6px;"></div>
      <div style="display:flex;align-items:center;justify-content:flex-end;gap:8px;flex-wrap:wrap;">
        <button class="tbtn" onclick="closeDeleteConfirmModal()">Cancelar</button>
        <button class="tbtn" style="background:rgba(220,80,80,.18);border-color:rgba(220,80,80,.35);color:#ffb7b7;" onclick="submitDeleteConfirmModal()">Eliminar</button>
      </div>
    </div>
  </div>
</div>
<div id="dateEditModal" style="display:none;position:fixed;inset:0;z-index:86;background:rgba(0,0,0,.68);backdrop-filter:blur(2px);">
  <div style="max-width:520px;width:calc(100% - 24px);margin:14vh auto;background:linear-gradient(180deg, rgba(24,28,36,.98), rgba(14,18,24,.98));border:1px solid var(--border);border-radius:14px;overflow:hidden;display:flex;flex-direction:column;box-shadow:0 24px 70px rgba(0,0,0,.45);">
    <div style="display:flex;align-items:center;justify-content:space-between;padding:12px 14px;border-bottom:1px solid var(--border);">
      <strong style="font-size:13px;">Corregir fecha</strong>
      <button class="tbtn" onclick="closeDateEditModal()">Cerrar</button>
    </div>
    <div style="padding:14px;display:flex;flex-direction:column;gap:12px;">
      <div style="font-size:12px;color:var(--muted-l);line-height:1.45;">
        <div id="dateEditCount" style="font-family:var(--mono);color:var(--text);margin-bottom:6px;">0 archivos</div>
        <div>Escribe una fecha válida en formato <span style="font-family:var(--mono);color:var(--text);">YYYY-MM-DD</span>. Las fechas futuras se bloquean.</div>
      </div>
      <label for="dateEditInput" style="font-size:12px;color:var(--muted-l);">Nueva fecha</label>
      <input id="dateEditInput" type="date" style="width:100%;background:var(--card);border:1px solid var(--border);color:var(--text);border-radius:10px;padding:10px 12px;font-family:var(--mono);font-size:13px;outline:none;">
      <div style="display:flex;align-items:center;justify-content:flex-end;gap:8px;flex-wrap:wrap;">
        <button class="tbtn" onclick="closeDateEditModal()">Cancelar</button>
        <button class="tbtn active" onclick="submitDateEditModal()">Guardar cambio</button>
      </div>
    </div>
  </div>
</div>
...
## File: server/client-http/index.html
````html
<!doctype html>
<html lang="es">
<head>
...
````json
{
  ...
  "port": 3000,
  "sourceMode": "filesystem",
  "mediaRoot": "C:\\Users\\vCesar\\OneDrive\\Escritorio\\takeout-20260520T011015Z-3-001\\Takeout",
  "mediaRoots": [
    "C:\\Users\\vCesar\\OneDrive\\Escritorio\\takeout-20260520T011015Z-3-001\\Takeout"
  ],
  "autoindexRootUrl": "http://192.168.1.125:8080/takeout-20260520T011015Z-3-001/",
  "autoindexRootUrls": [
    "http://192.168.1.125:8080/takeout-20260520T011015Z-3-001/"
...
## File: server/js/features/thumb/handler.mjs
````javascript
async function runCommand(command, args)
⋮----
function createThumbHandler(ctx)
...
## File: server/js/shared/media-utils.mjs
````javascript
...
## File: server/client-http/js/app/main.js
````javascript
...
## File: server/js/features/indexing/builders.mjs
````javascript
async function walk(fs, dir, out = [])
⋮----
async function runCommand(command, args)
...
## File: server/js/features/live/service.mjs
````javascript
function createLiveService(ctx)
⋮----
function resolveFilesystemMediaSrc(src)
