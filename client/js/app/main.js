...
let livePhotoKitPlayer = null;
let livePrepareRetryTimer = null;
let livePrepareRetryCount = 0;
...
function toggleFavoritesOnly() {
  ...
  updateFavOnlyBtn();
  applyFilters({ forceGridReload: true });
}
...
function updateSelectionUI() {
  ...
  const fixBtn = document.getElementById("fixSelectedBtn");
  const fixLbl = document.getElementById("fixSelectedLbl");
  ...
  if (fixBtn) fixBtn.style.display = selectionMode ? "flex" : "none";
  if (fixLbl)
    fixLbl.textContent =
      selectedUrls.size > 1
        ? `Corregir fecha (${selectedUrls.size})`
        : "Corregir fecha";
...
function getSelectedFiles() {
  const byUrl = new Map(allFiles.map((f) => [f?.url, f]));
...
async function submitDateEditModal() {
  ...
  if (date > today) {
    toast("La fecha no puede ser futura.");
    return;
  }
  const btn = document.getElementById("fixSelectedBtn");
  const prevDisabled = btn?.disabled || false;
  if (btn) btn.disabled = true;
  try {
    const res = await fetch(UPDATE_MEDIA_DATE_URL, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        urls: selectedFiles.map((f) => f.url),
        date,
      }),
    });
    const payload = await res.json().catch(() => ({}));
    if (!res.ok) throw new Error(payload.error || `HTTP ${res.status}`);
    toast(
      `Fecha actualizada en ${payload.updated || selectedFiles.length} archivo${(payload.updated || selectedFiles.length) === 1 ? "" : "s"}. Reindexando...`,
      3200,
    );
    try {
...
async function fetchThumbWithRetry(url, retries = THUMB_PREWARM_RETRIES) {
  ...
  while (attempt <= retries) {
    try {
      if (thumbLoadingSuspended) throw new Error("suspended");
      const res = await fetch(toRemoteUrl(url), { cache: "no-store" });
  ...
  for (const f of files) {
    ...
    const u =
      typeof f.thumbUrl === "string" && f.thumbUrl.trim()
        ? f.thumbUrl.trim()
        : toRemoteUrl(`/thumb/video?src=${encodeURIComponent(f.url)}&v=0`);
    if (!u) continue;
...
function clampDateToToday(dateStr) {
  const src = String(dateStr || "").trim();
  if (!isValidFullDateString(src)) return todayDateString();
  const today = todayDateString();
...
async function loadConfigModalData() {
  try {
    ...
    const remoteInput = document.getElementById("cfgRemoteUrl");
    if (remoteInput) {
      const fallback = String(window.media3xvRuntime?.baseUrl || "");
      let remoteUrl = fallback;
      if (typeof window.media3xvRuntime?.getClientConfig === "function") {
        try {
          const clientCfg = await window.media3xvRuntime.getClientConfig();
          remoteUrl = String(clientCfg?.remoteUrl || fallback);
        } catch {
          remoteUrl = fallback;
        }
      }
      remoteInput.value = remoteUrl;
    }
  } catch (err) {
...
async function saveConfigModalData() {
  ...
  const remoteUrl = String(
    document.getElementById("cfgRemoteUrl")?.value || "",
  ).trim();
  try {
    ...
    if (!res.ok) throw new Error(payload.error || `HTTP ${res.status}`);
    if (typeof window.media3xvRuntime?.setClientConfig === "function") {
      await window.media3xvRuntime.setClientConfig({ remoteUrl });
    }
    closeConfigModal();
    toast("Configuración guardada. Reinicia la app para aplicar remoteUrl.", 5200);
  } catch (err) {
...
function parsePartitionKey(key) {
  const m = String(key || "").match(/^fs:(\d+):(.*):(\d{4}-\d{2}|0000-00)$/);
  if (!m) return { key: String(key || ""), rootIdx: "?", folderGroup: "", month: "" };
  return {
    key: String(key || ""),
    rootIdx: Number(m[1]),
    folderGroup: m[2],
...
async function reindexPartitionByKey(partitionKey) {
  ...
  if (!key) return;
  if (!window.confirm(`¿Reindexar partición?\n${key}`)) return;
  try {
    const res = await fetch(REINDEX_PARTITION_URL, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ partitionKeys: [key] }),
    });
    const payload = await res.json().catch(() => ({}));
    if (!res.ok) throw new Error(payload.error || `HTTP ${res.status}`);
    toast(`Reindexado iniciado para ${payload.forced || 1} partición(es)`, 3200);
  } catch (err) {
    toast(`No se pudo reindexar partición: ${err.message}`, 4200);
  }
}

async function loadPartitionsList() {
  const box = document.getElementById("partitionsBox");
  if (!box) return;
  box.innerHTML =
    '<div style="font-size:11px;color:var(--muted);">Cargando particiones...</div>';
  try {
    const res = await fetch(PARTITIONS_URL, { cache: "no-store" });
    const payload = await res.json().catch(() => ({}));
    if (!res.ok) throw new Error(payload.error || `HTTP ${res.status}`);
    const rows = Array.isArray(payload.partitions) ? payload.partitions : [];
    if (!rows.length) {
      box.innerHTML =
        '<div style="font-size:11px;color:var(--muted);">No hay particiones disponibles.</div>';
      return;
    }
    box.innerHTML = "";
    for (const row of rows) {
      ...
      item.style.cssText =
        "display:flex;align-items:center;gap:8px;justify-content:space-between;border:1px solid var(--border);border-radius:7px;padding:6px 8px;background:var(--panel);";
      const left = document.createElement("div");
      left.style.cssText = "display:flex;flex-direction:column;gap:2px;min-width:0;";
      left.innerHTML = `
        <div style="font-size:11px;color:var(--text);font-family:var(--mono);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" title="${key}">${key}</div>
        <div style="font-size:10px;color:var(--muted);">root=${parsed.rootIdx} folder=${parsed.folderGroup || "root"} month=${parsed.month} · count=${Number(row?.count || 0)}</div>
      `;
      const btn = document.createElement("button");
      btn.className = "tbtn";
      btn.textContent = "Reindexar";
      btn.onclick = () => reindexPartitionByKey(key);
    ...
    box.innerHTML =
      '<div style="font-size:11px;color:var(--danger);">Error cargando particiones.</div>';
    toast(`No se pudo cargar particiones: ${err.message}`, 4200);
  }
}
...
function clearLivePrepareRetryTimer() {
  if (!livePrepareRetryTimer) return;
  clearTimeout(livePrepareRetryTimer);
  livePrepareRetryTimer = null;
}

function destroyLivePhotoKitPlayer() {
  const host = document.getElementById("viewerLivePhotoPlayer");
  if (!host) return;
  host.onclick = null;
  host.onmouseenter = null;
  host.ontouchstart = null;
  try {
    if (livePhotoKitPlayer && typeof livePhotoKitPlayer.stop === "function") {
      livePhotoKitPlayer.stop();
    }
  } catch (_) {}
  livePhotoKitPlayer = null;
  host.innerHTML = "";
...
function fallbackToDirectVideoPlayback(file, videoEl) {
  ...
  if (forceWebForMov) {
    ...
    clearLivePrepareRetryTimer();
    const nextDelay = Math.min(7000, 900 + livePrepareRetryCount * 700);
    if (livePrepareRetryCount >= 8) {
      setLiveHint("No se pudo convertir este MOV temporalmente. Intenta descargarlo.");
      return;
    }
...
async function startLivePrepareAndPoll(
  ...
  try {
    ...
    if (!prepRes.ok || prepPayload?.error) {
      videoEl.dataset.livePreparing = "0";
      livePrepareInFlightKey = "";
      const msg = String(prepPayload?.error || "").trim();
      setLiveHint(
        msg ? `Fallo de conversión: ${msg}` : "No se pudo iniciar la conversión",
      );
      return false;
    }
    ...
    liveStatusTimer = setInterval(async () => {
      try {
        ...
        if (statusPayload?.error) {
          ...
          videoEl.dataset.livePreparing = "0";
          livePrepareInFlightKey = "";
          const msg = String(statusPayload.error || "").trim();
          setLiveHint(
            msg
              ? `Fallo de conversión: ${msg}`
              : "Falló la conversión temporal del video",
          );
          return;
        }
...
function normalizeDate(dateStr, fallbackName, fallbackUrl) {
  ...
  if (/^\d{4}-\d{2}-\d{2}$/.test(src)) {
    const value = clampDateToToday(src);
    return { value, fallback: value !== src };
  }
  const yearOnly = src.match(/^(\d{4})-XX-XX$/);
  if (yearOnly) {
...
function renderViewer() {
  ...
  const img = document.getElementById("viewerMedia");
  const livePlayerHost = document.getElementById("viewerLivePhotoPlayer");
  const vid = document.getElementById("viewerVideo");
  const aud = document.getElementById("viewerAudio");
  img.style.display = "none";
  if (livePlayerHost) livePlayerHost.style.display = "none";
  vid.style.display = "none";
  aud.style.display = "none";
  ...
  if (f.type === "image") {
    ...
    const targetUrl = isHeic
      ? toRemoteUrl(`/thumb/web-image?src=${encodeURIComponent(f.url)}`)
      : toRemoteUrl(f.url);
    ...
    document.getElementById("viewerAudioEl").src = toRemoteUrl(f.url);
  }
}
...
const THUMB_DB_BASE_KEY = "media3xv:thumb-db-base";
...
async function getThumbObjectUrl(
  ...
  const res = await fetch(toRemoteUrl(src), { cache: "default", signal });
...
async function clearThumbDb() {
  ...
  thumbDbPromise = null;
  await new Promise((resolve) => {
    const req = indexedDB.deleteDatabase(THUMB_DB_NAME);
...
async function ensureThumbCacheForCurrentBase() {
  const base = RUNTIME_BASE_URL || location.origin || "file://";
  const previous = localStorage.getItem(THUMB_DB_BASE_KEY) || "";
  if (previous === base) return;
  await clearThumbDb();
  localStorage.setItem(THUMB_DB_BASE_KEY, base);
}
...
function applyFilters(opts = {}) {
  const forceGridReload = Boolean(opts?.forceGridReload);
...
// ── INIT ──────────────────────────────────────────────────────
(async () => {
  await ensureThumbCacheForCurrentBase();
